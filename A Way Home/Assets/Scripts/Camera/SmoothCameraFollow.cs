...
public class SmoothCameraFollow : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        Vector3 _idealPosition = _boat.transform.position + (_boat.transform.forward * _distanceOffset) + (_boat.transform.up * _heightOffset);
       
        // Find how far away we are from the ideal position
        Vector3 _displacement = transform.position - _idealPosition;
        
        // Calculate how fast we should be moving towards our initial position using displacement and velocity
        Vector3 _acceleration = (-_springConstant * _displacement) - (_dampConstant * _velocity);
        ...
        /*Vector3 whereShouldIbe = new Vector3(
            transform.position.x + _velocity.x,
            transform.position.y + _velocity.y,
            transform.position.z + _velocity.z);

        Vector3 _smoothedPosition = Vector3.Slerp(transform.position, whereShouldIbe, _smoothSpeed);
